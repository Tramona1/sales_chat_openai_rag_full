...
// Assuming utils directory is at the root level alongside get_vector.cts
const { getEmbeddingClient } = require('./utils/embeddingClient'); // Use require for CommonJS
// Optionally import AI_SETTINGS if needed to check the provider
// const { AI_SETTINGS } = require('./utils/modelConfig'); // Assuming modelConfig.js exists
...
async function generateAndPrintVector() {
  ...
  } catch (error: any) { // Added :any for type safety in catch
    console.error("\n--- ERROR ---");
