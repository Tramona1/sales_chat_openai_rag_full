import { TestCase } from './evaluation';
...
export const PRICING_QUERIES = TEST_QUERIES.filter(q => q.category === 'pricing');
export const FEATURE_QUERIES = TEST_QUERIES.filter(q => q.category === 'features');
export const COMPETITOR_QUERIES = TEST_QUERIES.filter(q => q.category === 'competitors');
export const TECHNICAL_QUERIES = TEST_QUERIES.filter(q => q.category === 'technical');
export const GENERAL_QUERIES = TEST_QUERIES.filter(q => q.category === 'general');

// Export queries grouped by complexity for focused testing
export const SIMPLE_QUERIES = TEST_QUERIES.filter(q => q.complexity === 1);
export const MEDIUM_QUERIES = TEST_QUERIES.filter(q => q.complexity === 2);
export const COMPLEX_QUERIES = TEST_QUERIES.filter(q => q.complexity === 3); 