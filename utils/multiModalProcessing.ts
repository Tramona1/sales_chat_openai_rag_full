...
export async function createMultiModalChunks(
  ...
  try {
    ...
    const textChunks = await splitIntoChunksWithContext(
      documentText,
      500,
      source,
      true,
...
export async function generateMultiModalEmbeddings(
  chunks: MultiModalVectorStoreItem[]
): Promise<MultiModalVectorStoreItem[]> {
  try {
 ...
 */
export async function analyzeDocumentVisuals(filePath: string): Promise<DocumentVisualAnalysis> {
  try {
    const startTime = Date.now();
    ...
    // Analyze each image
    const analyzedImages: ProcessedImage[] = await Promise.all(
...
export async function performMultiModalSearch(
  ...
  try {
    ...
    let visualTypes: VisualContentType[] | undefined = options.visualTypes;
    ...
    if (visualFocus || includeVisualContent) {
      ...
      if (visualTypes && visualTypes.length > 0) {
        const typedResults = filteredResults.filter(result => {
          ...
          const visualContent = (result.item as any).visualContent;
          ...
          if (Array.isArray(visualContent)) {
            return visualContent.some(vc => 
              visualTypes?.some(vt => vt.toString() === vc.type)
            );
          }
          ...
          if (metadata.visualElementTypes && Array.isArray(metadata.visualElementTypes)) {
            return metadata.visualElementTypes.some((type: string) => 
              visualTypes?.some(vt => vt.toString() === type)
            );
          }
        ...
        filteredResults = filteredResults.map(result => {
          ...
          // Text chunks with embedded visuals get a medium boost
            ...
            if (visualTypes && Array.isArray(visualContent)) {
              const hasMatchingType = visualContent.some(vc => 
                visualTypes?.includes(vc.type)
              );
    ...
    const multiModalResults = filteredResults.map(result => {
      ...
      if (Array.isArray(visualContent) && visualContent.length > 0) {
        ...
        if (visualTypes && visualTypes.length > 0) {
          const typeMatches = visualContent.filter(vc => 
            visualTypes?.includes(vc.type)
          );
        ...
        if (!bestVisual) {
          const contentMatches = visualContent.filter(vc => 
            (vc.description && vc.description.toLowerCase().includes(query.toLowerCase())) ||
