...
export interface FtsSearchOptions {
  ...
  /** Content column to search (default: content) */
  contentColumn?: string;
  /** Table to search (default: document_chunks) */
  table?: string;
 ...
 */
export async function performFtsSearch(
  query: string,
  options: FtsSearchOptions = {}
): Promise<Array<VectorStoreItem & { score: number }>> {
  const {
    limit = 10,
    threshold = 0.1,
 ...
 */
function createTsQuery(query: string): string {
