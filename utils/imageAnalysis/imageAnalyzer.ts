...
import { VisualContentType, ImageAnalysisResult as BaseImageAnalysisResult } from '../../types/baseTypes';
...
export interface ImageAnalysisResult extends BaseImageAnalysisResult {
  ...
  // Ensure we have access to data property
  data?: any;
  
  // Extracted text from the image (non-optional to match base interface)
 ...
 */
function mapImageTypeToVisualContentType(imageType: ImageType): VisualContentType {
  const typeMap: Record<string, VisualContentType> = {
...
export interface ImageAnalysisOptions {
  ...
  // Whether to perform OCR on the image
  extractText?: boolean;
  
  // Model to use (defaults to gemini-2.0-flash)
  model?: 'gemini-2.0-flash' | 'gemini-pro-vision';
  
  // Temperature for generation (defaults to 0.2)
  temperature?: number;
}

export interface AnalyzedVisualElement {
  pageNumber: number;
  visualId?: string;
  storageUrl?: string;
...
Provide your response in this exact JSON format:
{
  ...
  "confidence": 0.95
      ...
      const analysisResult: AnalyzedVisualElement = {
        pageNumber,
  ...
  private validateVisualType(type?: string): AnalyzedVisualElement['visualType'] {
    const validTypes: AnalyzedVisualElement['visualType'][] = [
    ...
    return type as AnalyzedVisualElement['visualType'];
  }
  ...
  public static async analyzeImage(
    ...
    try {
      ...
      const model = genAI.getGenerativeModel({
        model: analysisOptions.model as string,
        safetySettings: [
          {
...
Reply ONLY with a valid JSON object containing these keys and relevant values.`;
      ...
      if (jsonStart >= 0 && jsonEnd >= 0) {
        ...
        try {
          ...
          // Format the result
          const visualType = mapImageTypeToVisualContentType(
            this.determineImageType(analysisResult.IMAGE_TYPE)
          );
   ...
   */
  private static determineImageType(typeText: string): ImageType {
   ...
   */
  public static generateDocumentContext(analysis: ImageAnalysisResult): any {
   ...
   */
  private static extractTopicsFromAnalysis(analysis: ImageAnalysisResult): string[] {
    const topics: string[] = [];
   ...
   */
  private static extractEntitiesFromAnalysis(analysis: ImageAnalysisResult): string[] {
   ...
   * (0-3 scale)
   */
  private static assessTechnicalLevel(analysis: ImageAnalysisResult): number {
    const fullText = analysis.description + ' ' + (analysis.detectedText || '');
   ...
   */
  private static determineAudienceType(analysis: ImageAnalysisResult, technicalLevel: number): string[] {
    const fullText = analysis.description + ' ' + (analysis.detectedText || '');
    const lowerText = fullText.toLowerCase();
   ...
   */
  public static generateChunkContext(analysis: ImageAnalysisResult): any {
    return {
   ...
   */
  private static extractKeyPointsFromAnalysis(analysis: ImageAnalysisResult): string[] {
   ...
   */
  public static prepareTextForEmbedding(analysis: ImageAnalysisResult): string {
