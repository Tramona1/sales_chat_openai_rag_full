...
const Drawer = ({ children, open = false, onClose, variant = 'temporary', anchor = 'left', className = '', sx = {}, ModalProps, }) => {
    ...
    if (variant === 'permanent' || (variant !== 'temporary' && open)) {
        return (<div className={`bg-white shadow-xl fixed ${positionClasses[anchor]} ${sizeClasses[anchor]} overflow-auto z-30 ${className}`} style={drawerStyles}>
        {children}
      </div>);
    }
    ...
    if (variant === 'temporary' && open) {
        return (<>
        {/* Backdrop */}
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity" onClick={handleBackdropClick}/>
