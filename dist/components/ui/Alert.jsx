...
const Alert = ({ children, severity = 'info', onClose, variant = 'standard', className = '' }) => {
    ...
    const renderIcon = () => {
        switch (severity) {
            case 'error':
                return (<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    // Color styles based on severity and variant
    const getColorStyles = () => {
        const colors = {
            error: {
                standard: 'bg-red-50 text-red-800 border-red-200',
                filled: 'bg-red-600 text-white',
                outlined: 'bg-white text-red-800 border border-red-300'
            },
            warning: {
                standard: 'bg-yellow-50 text-yellow-800 border-yellow-200',
    ...
    return (<div className={`flex items-center p-4 rounded-md ${getColorStyles()} ${className}`} role="alert">
      <div className="flex-shrink-0 mr-3">
        {renderIcon()}
      </div>
      <div className="flex-grow">{children}</div>
      {onClose && (<button type="button" className="ml-auto flex-shrink-0 focus:outline-none" onClick={onClose} aria-label="Close">
