    ...
    return (<div className="w-full">
      <form onSubmit={handleSubmit} className="mb-4">
        <div className="flex flex-col space-y-2">
          ...
          <div className="flex">
            <input id="company-name" type="text" className={`flex-grow px-4 py-2 border ${darkMode
            ? 'bg-[#3e3f4b] border-gray-600 text-white focus:ring-blue-500 focus:border-blue-500'
            : 'border-gray-300 focus:ring-blue-500 focus:border-blue-500'} rounded-l-lg`} placeholder="Enter company name..." value={companyName} onChange={(e) => onChange(e.target.value)} onKeyDown={handleKeyDown} disabled={isSearching}/>
            <button type="submit" className={`px-4 py-2 rounded-r-lg text-white ${isSearching
            ? 'bg-blue-400 cursor-not-allowed'
            : 'bg-blue-600 hover:bg-blue-700'}`} disabled={isSearching}>
      ...
      {error && (<div className={`p-3 ${darkMode ? 'bg-red-900/50 border-red-800 text-red-200' : 'bg-red-50 border-red-200 text-red-600'} border rounded-md text-sm mb-4`}>
          {error}
        </div>)}
