...
function UploadForm({ onSuccess, onError }) {
    ...
    const getFileIcon = (file) => {
        if (file.type.includes('image')) {
          ...
          <path d="M3 8a2 2 0 012-2h2a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8z"/>
            ...
            return (<svg className="w-5 h-5 mr-2 text-blue-700" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd"/>
        </svg>);
        }
        else {
            return (<svg className="w-5 h-5 mr-2 text-gray-500" fill="currentColor" viewBox="0 0 20 20">
    ...
    return (<div className="w-full max-w-3xl mx-auto">
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
        ...
        {formState.error && (<div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
            <p>{formState.error}</p>
          </div>)}
        
        {formState.success && (<div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
        ...
        {formState.files.length > 0 && (<div className="mb-4">
            <h3 className="text-md font-medium mb-2">Selected Files</h3>
            <ul className="bg-gray-50 rounded-md divide-y divide-gray-200">
              {formState.files.map((file, index) => (<li key={index} className="flex items-center justify-between py-3 px-4">
        ...
        <div className="space-y-3 mb-4">
          <div className="flex items-center">
            <input id="contextualProcessing" type="checkbox" checked={formState.contextualProcessing} onChange={e => setFormState(prev => ({ ...prev, contextualProcessing: e.target.checked }))} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
          ...
          <div className="flex items-center">
            <input id="visualProcessing" type="checkbox" checked={formState.visualProcessing} onChange={e => setFormState(prev => ({ ...prev, visualProcessing: e.target.checked }))} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"/>
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={formState.isUploading || formState.files.length === 0} className={`inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${(formState.isUploading || formState.files.length === 0) ? 'opacity-50 cursor-not-allowed' : ''}`}>
