...
const SearchResults = ({ results, query, searchType, executionTimeMs, sessionId, userId, highlightedTags = [], onResultClick }) => {
    ...
    // Handle result click
    const handleResultClick = (resultId, index) => {
        if (onResultClick) {
    ...
    if (results.length === 0) {
        return (<div className="p-4 my-4 bg-gray-50 border border-gray-200 rounded-lg">
        <p className="text-gray-600 text-center">No results found for your search.</p>
      </div>);
    }
    return (<div className="space-y-4 my-4">
      ...
      {results.map((result, index) => {
            ...
            return (<SearchResultTracker key={result.id} resultId={result.id} query={query} index={index} sessionId={sessionId} userId={userId}>
            <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={() => handleResultClick(result.id, index)}>
              <h3 className="text-lg font-medium text-gray-900 mb-1">{result.title || 'Untitled'}</h3>
              
              <div className="flex flex-wrap gap-1 mb-2">
                {/* Source and score info */}
                {result.source && (<p className="text-xs text-gray-500">
                    Source: {result.source}
                    {result.score !== undefined && ` • Score: ${result.score.toFixed(2)}`}
                  </p>)}
                
                {/* Technical level badge */}
                {technicalLevel && (<span className={`ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${highlightedTags.includes(technicalLevel)
              ...
              {/* Tags display */}
              {(topics.length > 0 || audience.length > 0 || entities.length > 0) && (<div className="flex flex-wrap gap-1 mt-3">
                  {/* Topic tags */}
                  {topics.map(topic => (<span key={`topic-${topic}`} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${highlightedTags.includes(topic)
                  ...
                  {/* Audience tags */}
                  {audience.map(type => (<span key={`audience-${type}`} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${highlightedTags.includes(type)
                  ...
                  {/* Entity tags */}
                  {entities.map(entity => (<span key={`entity-${entity}`} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${highlightedTags.includes(entity)
              ...
              {result.metadata && Object.keys(result.metadata).length > 0 && (<div className="mt-3 pt-2 border-t border-gray-100">
