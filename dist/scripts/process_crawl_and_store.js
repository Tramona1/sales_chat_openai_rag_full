...
const logger = {
    ...
    warning: (message, ...args) => console.warn(`[WARN] ${message}`, ...args),
    debug: (message, ...args) => console.debug(`[DEBUG] ${message}`, ...args),
    success: (message, ...args) => console.log(`[SUCCESS] ${message}`, ...args),
};
// --- Helper: Read Source Data (from JSON files in a directory) ---
function getCrawlData(loggerInstance, startIndex = 0, endIndex = Infinity) {
    loggerInstance.info(`Reading crawl data from directory: ${CRAWL_DATA_DIR}`);
    if (!fs.existsSync(CRAWL_DATA_DIR)) {
        loggerInstance.error(`Crawl data directory not found: ${CRAWL_DATA_DIR}`);
        return [];
    }
    const allData = [];
    try {
        const files = fs.readdirSync(CRAWL_DATA_DIR);
        ...
        for (const file of jsonFiles) {
            const filePath = path.join(CRAWL_DATA_DIR, file);
            try {
                const fileContent = fs.readFileSync(filePath, 'utf-8');
                const parsedData = JSON.parse(fileContent); // Assuming each file contains one object
        ...
        loggerInstance.error(`Failed to read directory: ${CRAWL_DATA_DIR}`, error);
        return [];
    }
}
...
async function rebuildVectorStore() {
    ...
    try {
        ...
        // --- Load Source Data ---
        let sourceDocs = getCrawlData(logger, startIndex, endIndex);
        ...
        for (let i = 0; i < sourceDocs.length; i += PROCESSING_BATCH_SIZE) {
            ...
            const batchPromises = batchDocs.map(async (doc) => {
                ...
                try {
                    ...
                    const analysisResult = await analyzeDocument(doc.content, doc.url, { useCaching: false });
                    ...
                    const chunks = await splitIntoChunksWithContext(doc.content, DEFAULT_CHUNK_SIZE, doc.url, true, analysisResult.documentContext);
