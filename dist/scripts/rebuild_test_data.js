...
const metadataExtractor_1 = require("../utils/metadataExtractor");
const openaiClient_1 = require("../utils/openaiClient");
const vectorStore_1 = require("../utils/vectorStore");
const bm25_1 = require("../utils/bm25");
// Constants
const TEST_DATA_DIR = path_1.default.join(process.cwd(), 'test_data');
const DOCS_DIR = path_1.default.join(TEST_DATA_DIR, 'documents');
const TEST_QUERIES_PATH = path_1.default.join(TEST_DATA_DIR, 'test_queries.json');
/**
 * Update test queries to reference Workstream instead of SalesBuddy
 */
async function updateTestQueries() {
    ...
    try {
        ...
        await promises_1.default.writeFile(TEST_QUERIES_PATH, JSON.stringify(workstreamQueries, null, 2));
...
async function updateProductOverview() {
    ...
    const productOverviewPath = path_1.default.join(DOCS_DIR, 'product_overview.md');
    try {
        // Read existing content to see what might need to be preserved
        let existingContent = '';
        try {
            existingContent = await promises_1.default.readFile(productOverviewPath, 'utf-8');
        }
        catch (err) {
...
*Note: This document is for internal training and testing purposes only.*`;
        ...
        await promises_1.default.writeFile(productOverviewPath, workstreamOverview);
...
async function updateApiDocumentation() {
    ...
    const apiDocPath = path_1.default.join(DOCS_DIR, 'api_documentation.md');
    try {
...
*Note: This documentation is for internal testing purposes only.*`;
        ...
        await promises_1.default.writeFile(apiDocPath, workstreamApiDoc);
...
async function indexTestDocuments() {
    ...
    try {
        ...
        const files = await promises_1.default.readdir(DOCS_DIR);
        ...
        for (const file of docFiles) {
            ...
            const metadata = await (0, metadataExtractor_1.extractMetadata)(content, documentId);
            // Generate embedding
            console.log(`Generating embedding for ${file}...`);
            const embedding = await (0, openaiClient_1.embedText)(content);
            ...
            // Add to vector store
            await (0, vectorStore_1.addToVectorStore)(vectorItem);
...
async function main() {
    ...
    try {
        ...
        await promises_1.default.mkdir(TEST_DATA_DIR, { recursive: true });
        await promises_1.default.mkdir(DOCS_DIR, { recursive: true });
        // Update test queries
