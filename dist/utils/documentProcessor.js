...
import { extractText } from './documentProcessing';
 ...
 */
export async function processDocument(input, source, mimeType, options = {}) {
    ...
    try {
        ...
        const chunks = await splitIntoChunksWithContext(documentText, chunkSize, source, true, // Generate context for chunks
        ...
        const vectorItems = chunksWithVisuals.map((chunk, index) => {
 ...
 */
export async function batchProcessDocuments(documents, options = {}, concurrency = 3) {
    const results = [];
    const queue = [...documents];
    // Process in batches based on concurrency
    while (queue.length > 0) {
        const batch = queue.splice(0, concurrency);
        const batchPromises = batch.map(doc => processDocument(doc.input, doc.source, doc.mimeType, options));
