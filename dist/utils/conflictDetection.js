...
async function detectConflictsWithGemini(documents, entityName) {
    ...
    const patternBasedGroups = detectDocumentConflicts(documents, entityName);
    ...
    for (const doc of documents) {
        ...
        const category = (doc.metadata && 'category' in doc.metadata ? doc.metadata.category : null) ||
            (doc.metadata && 'primaryCategory' in doc.metadata ? doc.metadata.primaryCategory : null) ||
            'uncategorized';
    ...
    for (const [category, docs] of Object.entries(docsByCategory)) {
        if (docs.length < 2)
            continue; // Need at least 2 docs to have a conflict
...
function detectLeadershipConflicts(documents) {
    ...
    const sortedMentions = [...ceoMentions].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
 ...
 */
function detectPricingConflicts(documents) {
 ...
 */
function detectFeatureConflicts(documents) {
