 ...
 */
export function capitalizeCompanyName(companyName) {
    if (!companyName)
        return '';
 ...
 */
export async function generateCompanySuggestions(misspelledName) {
    try {
 ...
 */
function generateLocalSuggestions(companyName) {
 ...
 */
export async function verifyCompanyIdentity(companyName) {
    try {
        ...
        if (!supabaseError && supabaseCache && new Date(supabaseCache.expires_at) > new Date()) {
            // Get the stored data
            const cachedResult = supabaseCache.data;
            // If the result shows company doesn't exist and has no suggestions, 
            // or has empty suggestions array, try to generate suggestions now
            if (!cachedResult.exists &&
                (!cachedResult.suggestions || cachedResult.suggestions.length === 0)) {
                ...
                if (suggestions && suggestions.length > 0) {
                    ...
                    cacheWithExpiry(cacheKey, updatedResult, new Date(supabaseCache.expires_at).getTime() - Date.now());
            ...
            cacheWithExpiry(cacheKey, cachedResult, new Date(supabaseCache.expires_at).getTime() - Date.now());
 ...
 */
export async function getCompanyInformation(companyName, options = {}) {
    try {
        ...
        if (!options.forceRefresh) {
            ...
            if (!supabaseError && supabaseCache && new Date(supabaseCache.expires_at) > new Date()) {
                // Cache the data in memory too
                cacheWithExpiry(cacheKey, supabaseCache.data, new Date(supabaseCache.expires_at).getTime() - Date.now());
                logPerplexityUsage('getCompanyInformation.supabaseCacheHit', { companyName: displayName });
                return supabaseCache.data;
            }
        }
