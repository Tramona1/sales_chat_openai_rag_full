...
exports.runFullEvaluation = runFullEvaluation;
 ...
 */
async function evaluateRetrieval(testCases, retrievalFunction) {
    const results = [];
    for (const testCase of testCases) {
        ...
        if (testCase.expectedChunkIds && testCase.expectedChunkIds.length > 0) {
            ...
            const retrievedSources = retrievedItems.map(item => { var _a; return (_a = item.metadata) === null || _a === void 0 ? void 0 : _a.source; }).filter(Boolean);
            const uniqueRetrievedSources = Array.from(new Set(retrievedSources));
            const matchedSources = uniqueRetrievedSources.filter(source => testCase.expectedSourceDocs.some(expectedSource => source.includes(expectedSource)));
            ...
            const keywordMatches = testCase.expectedKeywords.filter(keyword => retrievedText.includes(keyword.toLowerCase()));
 ...
 */
async function evaluateAnswerQuality(testCase, generatedAnswer) {
...
Generated Answer: ${generatedAnswer}`;
    ...
    const getScores = async (modelName) => {
        try {
            // This is a placeholder - we'll need to implement the actual API calls
            // to the respective models when we have access to them
            const response = await generateChatCompletion(systemPrompt, userPrompt, modelName, true // Enable JSON mode
            );
 ...
 */
async function generateChatCompletion(systemPrompt, userPrompt, modelName, jsonMode = false) {
 ...
 */
async function runFullEvaluation(testCases, retrievalFunction, answerGenerationFunction) {
    // Evaluate retrieval
