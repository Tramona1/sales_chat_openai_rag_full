...
import { DocumentCategoryType, QualityControlFlag, ConfidenceLevel, EntityType, getCategoryAttributes, detectCategoryFromText } from './documentCategories';
import { logError, logInfo, logWarning } from './logger';
import { getFromCache, cacheWithExpiry } from './caching';
import { createDefaultMetadata } from '../types/metadata';
// Constants
 ...
 */
export async function analyzeDocument(text, source, options = {}) {
 ...
 */
async function analyzeDocumentWithLLM(text, model) {
...
`;
    try {
        ...
        if (model === 'gemini') {
            ...
            response = await generateStructuredGeminiResponse(systemPrompt, userPrompt, analysisSchema);
        }
        else {
            // Use OpenAI for structured response
            response = await generateStructuredResponse(systemPrompt, userPrompt, analysisSchema, model);
        }
 ...
 */
export async function batchAnalyzeDocuments(documents, options = {}) {
    const concurrency = options.concurrency || 3;
    const results = [];
    // Process in batches based on concurrency
    for (let i = 0; i < documents.length; i += concurrency) {
        const batch = documents.slice(i, i + concurrency);
        const batchPromises = batch.map(doc => analyzeDocument(doc.text, doc.source, options));
