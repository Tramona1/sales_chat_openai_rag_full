 ...
 */
export function toHybridSearchFilter(filter) {
    const hybridFilter = {};
    ...
    if (filter.entities && filter.entities.length > 0) {
        ...
        const requiredEntities = filter.entities.flatMap(entityGroup => entityGroup.names.map(name => `${entityGroup.type}:${name}`));
 ...
 */
export function extractStandardMetadata(document) {
 ...
 */
export function extractTagsFromResults(results) {
    const topicSet = new Map();
    const audienceSet = new Map();
    const entitySet = new Map();
    const technicalSet = new Map();
    ...
    const topicTags = Array.from(topicSet.entries()).map(([value, count]) => ({
        value,
        count,
        type: 'topic'
    }));
    const audienceTags = Array.from(audienceSet.entries()).map(([value, count]) => ({
        value,
        count,
        type: 'audience'
    }));
    const entityTags = Array.from(entitySet.entries()).map(([value, count]) => ({
        value,
        count,
        type: 'entity'
    }));
    const technicalTags = Array.from(technicalSet.entries()).map(([value, count]) => ({
        value,
        count,
        type: 'technical'
    }));
 ...
 */
export function getTopicsFromMetadata(metadata) {
    let topics = [];
 ...
 */
export function getAudienceFromMetadata(metadata) {
    let audience = [];
 ...
 */
export function getEntitiesFromMetadata(metadata) {
    let entities = [];
 ...
 */
export function getTechnicalLevelFromMetadata(metadata) {
 ...
 */
export function filterResultsByTags(results, selectedTags) {
    ...
    return results.filter(result => {
        ...
        return selectedTags.some(tag => topics.includes(tag) ||
 ...
 */
export function findTagsInQuery(query, availableTags) {
 ...
 */
export function parseTagFiltersFromQuery(query) {
