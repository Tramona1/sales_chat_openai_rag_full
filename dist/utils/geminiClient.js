...
let getModelForTask = async (config, task) => {
 ...
 */
export async function generateStructuredGeminiResponse(systemPrompt, userPrompt, responseSchema) {
 ...
 */
export async function generateGeminiChatCompletion(systemPrompt, userPrompt) {
    if (!apiKey || apiKey === 'your_google_ai_api_key_here') {
        throw new Error('GEMINI_API_KEY is not configured in environment');
    }
    try {
 ...
 */
export async function extractDocumentContext(documentText, metadata) {
 ...
 */
export async function generateChunkContext(chunkText, metadata) {
    // Get a limited version of the text to avoid token limits
 ...
 */
export async function embedTextWithGemini(text) {
