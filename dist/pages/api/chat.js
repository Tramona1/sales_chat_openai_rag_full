...
export default async function handler(req, res) {
    ...
    try {
        ...
        try {
            ...
            const response = await hybridSearch(userMessage, { limit: 30, matchThreshold: 0.7 });
            const searchResults = response.results;
            logDebug(`Retrieved ${searchResults.length} documents from hybrid search`);
            retrievedDocuments = searchResults.map((item) => ({
...
function constructOpenAISystemMessage(contextString, useAdmin, sourceList) {
...
function constructGeminiSystemMessage(contextString, useAdmin, sourceList) {
    let systemMessage = 'You are Workstream Sales Assistant, an AI designed to help with sales-related questions.\n\n';
    if (contextString) {
        systemMessage += 'CONTEXT INFORMATION:\n' + contextString + '\n\n';
    }
...
async function generateOpenAICompletion(messages, systemMessage, model) {
...
function processMessageForGemini(messages, systemMessage) {
...
async function generateGeminiCompletion(messages, systemMessage) {
