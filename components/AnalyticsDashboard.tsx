...
interface AnalyticsData {
  ...
  topReferencedContent: ContentReference[];
  lastUpdated: number;
...
const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ 
  ...
  const [selectedTab, setSelectedTab] = useState<'queries' | 'content' | 'sessions'>('queries');
  ...
  const getSessionPieChartData = () => {
    if (!analyticsData || !analyticsData.sessionStats) return [];
        ...
        <>
          ...
          {selectedTab === 'queries' && (
            <div>
              ...
              {analyticsData.commonQueries.length === 0 ? (
                <p className="text-gray-500 italic">No question data available yet.</p>
              ) : (
                <div className="h-96">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={getQueryBarData()}
                      layout="vertical"
          ...
          {selectedTab === 'content' && (
            <div>
              ...
              {analyticsData.topReferencedContent.length === 0 ? (
                <p className="text-gray-500 italic">No content reference data available yet.</p>
              ) : (
                <div className="h-96">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={getContentBarData()}
                      layout="vertical"
                      margin={{ top: 5, right: 30, left: 100, bottom: 5 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" />
          ...
          {selectedTab === 'sessions' && (
            <div>
              ...
              {!analyticsData.sessionStats ? (
                <p className="text-gray-500 italic">No session data available yet.</p>
              ) : (
                <>
                  <div className="mb-8">
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            ...
                            label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}
                          >
                            {getSessionPieChartData().map((entry, index) => (
                  ...
                  {analyticsData.sessionStats.companiesEngaged.length > 0 && (
                    <div>
                      <h4 className="text-md font-medium mb-2">Companies Engaged</h4>
                      <div className="bg-gray-50 p-4 rounded-lg">
                        <div className="flex flex-wrap gap-2">
                          {analyticsData.sessionStats.companiesEngaged.map((company, idx) => (
