...
interface Tag {
  value: string;
  count?: number;
  type: 'topic' | 'audience' | 'entity' | 'technical';
}
...
const TagFilterPanel: React.FC<TagFilterPanelProps> = ({
  ...
  const toggleSection = (section: 'topics' | 'audience' | 'entities' | 'technical') => {
    setExpandedSections({
      ...expandedSections,
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
      ...
      {topicTags.length > 0 && (
        <div className="mb-4">
          <button 
            className="flex w-full justify-between items-center text-sm font-medium text-gray-900 py-2"
            onClick={() => toggleSection('topics')}
          >
            Topics
            <svg 
              className={`h-5 w-5 transform ${expandedSections.topics ? 'rotate-180' : ''}`} 
              fill="none" 
              viewBox="0 0 24 24" 
              stroke="currentColor"
            >
      ...
      {audienceTags.length > 0 && (
        <div className="mb-4">
          ...
          >
            Audience
            <svg 
              className={`h-5 w-5 transform ${expandedSections.audience ? 'rotate-180' : ''}`} 
              fill="none" 
              viewBox="0 0 24 24" 
              stroke="currentColor"
            >
      ...
      {entityTags.length > 0 && (
        <div className="mb-4">
          ...
          >
            ...
            <svg 
              className={`h-5 w-5 transform ${expandedSections.entities ? 'rotate-180' : ''}`} 
              fill="none" 
              viewBox="0 0 24 24" 
              stroke="currentColor"
            >
      ...
      {technicalTags.length > 0 && (
        <div className="mb-4">
          ...
          >
            Technical Level
            <svg 
              className={`h-5 w-5 transform ${expandedSections.technical ? 'rotate-180' : ''}`} 
              fill="none" 
              viewBox="0 0 24 24" 
              stroke="currentColor"
            >
