...
const ChunkViewer: React.FC<ChunkViewerProps> = ({ documentId, documentTitle, onBack }) => {
  ...
  const [savingChunk, setSavingChunk] = useState<boolean>(false);
  ...
  const [regeneratingEmbedding, setRegeneratingEmbedding] = useState<boolean>(false);
  ...
  const handleRegenerateEmbedding = async (chunk: Chunk) => {
      ...
      renderCell: (row: Chunk) => (
        <div className="flex gap-2">
          <button
            ...
            onClick={() => handleEditChunk(row)}
          >
            <Edit size={16} />
          </button>
          <button
            className="p-1 text-green-600 hover:bg-green-50 rounded"
            onClick={() => handleRegenerateEmbedding(row)}
