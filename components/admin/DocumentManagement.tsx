...
// Document type matching our Supabase schema
interface Document {
  id: string;
  title: string;
  source: string;
...
// Document metadata editor form data
...
const DocumentManagement: React.FC = () => {
  ...
  const [viewChunksDialogOpen, setViewChunksDialogOpen] = useState<boolean>(false);
  const [editForm, setEditForm] = useState<DocumentMetadataForm>({
    title: '',
    source: '',
    primaryCategory: '',
    secondaryCategories: [],
  ...
  const handleNavigateToChunks = (documentId: string) => {
  ...
  const columns = [
    ...
    { 
      ...
      headerName: 'Category', 
      flex: 1,
      valueGetter: (row: Document) => row.metadata?.primaryCategory || 'Uncategorized'
    },
    { 
      field: 'approved', 
      headerName: 'Status', 
    ...
    { 
      ...
      width: 180,
      valueFormatter: (row: Document) => new Date(row.updated_at).toLocaleString()
    },
    { 
      field: 'actions', 
      headerName: 'Actions', 
    ...
    <div className="p-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
          ...
          <div className="md:col-span-2">
            <Button
              variant={searchByContent ? "contained" : "outlined"}
              color="primary"
      ...
      {error && (
        <div className="p-4 mb-4 border-l-4 border-red-500 bg-red-50 text-red-700">
          {error}
        </div>
      )}
      ...
      {editDialogOpen && selectedDocument && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              <div className="md:col-span-6">
                ...
                <select
                  ...
                  onChange={(e) => handleEditFormChange('primaryCategory', e.target.value)}
                >
              ...
              <div className="md:col-span-6">
                ...
                <select
                  className="w-full p-2 border border-gray-300 rounded"
                  value={editForm.technicalLevel}
                  onChange={(e) => handleEditFormChange('technicalLevel', parseInt(e.target.value))}
                >
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedDocument.metadata?.keywords && selectedDocument.metadata.keywords.length > 0 ? (
                    selectedDocument.metadata.keywords.map(keyword => (
                      <span key={keyword} className="bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-xs">
                        {keyword}
                      </span>
                    ))
                  ) : (
      ...
      {viewChunksDialogOpen && selectedDocument && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                className="text-gray-500 hover:text-gray-700"
                onClick={() => setViewChunksDialogOpen(false)}
              >
                <Close size={20} />
              </button>
            </div>
