...
import { ConflictGroup, ConflictType, formatDocumentSnippet } from '../../utils/conflictDetection';
...
export default function ContentConflictsPage() {
  ...
  const filteredConflicts = conflicts.filter(conflict => {
    ...
    if (currentTab === 'high-priority') return conflict.isHighPriority;
    if (currentTab === 'leadership') return conflict.topic.toLowerCase() === 'leadership';
    if (currentTab === 'pricing') return conflict.topic.toLowerCase() === 'pricing';
    if (currentTab === 'features') return conflict.topic.toLowerCase() === 'features';
    return true;
  });
  
  // Handle opening the resolve dialog
  const handleOpenResolveDialog = (conflict: ConflictGroup) => {
  ...
  const handleResolveConflict = async () => {
    if (!selectedConflict || !selectedDocId) return;
  ...
  const renderConflictBadge = (type: ConflictType) => {
    const styles: Record<ConflictType, { bg: string, text: string }> = {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {!loading && filteredConflicts.length > 0 && (
        <div className="space-y-6">
          {filteredConflicts.map((conflict, index) => (
            <Paper key={`${conflict.topic}-${index}`} className="overflow-hidden">
              <div className="px-6 py-4 bg-gray-50 border-b flex justify-between items-center">
                ...
                <Button
                  ...
                  disabled={resolvingConflict === conflict.topic}
                >
                  {resolvingConflict === conflict.topic ? (
                    <CircularProgress size={24} color="primary" className="mr-2" />
                  ) : null}
                  Resolve Conflict
                </Button>
              </div>
                  ...
                  <div key={i} className="mb-4">
                    ...
                    <div className="pl-4 border-l-4 border-gray-200">
                      {c.affectedDocIds.map((docId) => {
                        ...
                        return doc ? (
                          <div key={docId} className="mb-2 p-3 bg-gray-50 rounded">
      ...
      <Dialog
        ...
        title={`Resolve ${selectedConflict?.topic} Conflict`}
        actions={
          <>
            ...
            <Button
              ...
              disabled={!selectedDocId || resolvingConflict !== null}
            >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center mb-2">
                      <input
                        type="radio"
                        checked={selectedDocId === docId}
                        onChange={() => setSelectedDocId(docId)}
                    ...
                    <div className="ml-6">
                      ...
                      <div className="mt-2 text-xs text-gray-500">
                        {doc.metadata?.lastUpdated && (
                          <span>Last updated: {new Date(doc.metadata.lastUpdated).toLocaleString()}</span>
                        )}
                      </div>
                    </div>
                  </div>
