...
const Analytics: NextPage = () => {
  ...
  const [zeroResultQueries, setZeroResultQueries] = useState<TopSearch[]>([]);
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      ...
      try {
        ...
        const searchMetricsRes = await fetch(
          `/api/analytics/reports/search-metrics?start=${startDateStr}&end=${endDateStr}&type=${filters.searchType}`
        );
        
        // Fetch top searches
        const topSearchesRes = await fetch(
          `/api/analytics/reports/top-searches?start=${startDateStr}&end=${endDateStr}&limit=10`
        );
        
        // Fetch feedback summary
        const feedbackSummaryRes = await fetch(
          `/api/analytics/reports/feedback-summary?start=${startDateStr}&end=${endDateStr}`
        );
        
        // Fetch search trends over time
        const searchTrendsRes = await fetch(
          `/api/analytics/reports/search-trends?start=${startDateStr}&end=${endDateStr}&interval=day`
        );
        
        // Fetch zero result queries
        const zeroResultsRes = await fetch(
          `/api/analytics/reports/zero-results?start=${startDateStr}&end=${endDateStr}&limit=10`
        );
                              ...
                              <tr key={idx} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                                  <button 
                                    ...
                                    onClick={() => router.push(`/admin/content?suggest=${query.query_normalized}`)}
                                  >
                                    Add Content
                                  </button>
                                </td>
                      ...
                      <div className="space-y-4">
                        <div className="border border-gray-200 p-4 rounded-lg">
                          ...
                          <ul className="list-disc pl-5">
                            {zeroResultQueries.slice(0, 3).map((query, idx) => (
                              <li key={idx} className="text-gray-700">
                                {query.query_normalized}
                              </li>
                            ))}
                            {zeroResultQueries.length === 0 && (
                              <li className="text-gray-500">No content gaps detected in the selected period.</li>
                            )}
                          </ul>
                        </div>
                              ...
                              <>
                                ...
                                <li className="text-gray-700">
                                  Average search time is {
                                    searchMetrics.reduce((acc, curr) => acc + curr.avg_execution_time_ms, 0) / 
