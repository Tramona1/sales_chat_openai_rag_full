...
import { TEST_QUERIES, PRICING_QUERIES, FEATURE_QUERIES, COMPETITOR_QUERIES } from '../utils/test_queries';
import { runFullEvaluation, visualizeResults } from '../utils/evaluation';
import fs from 'fs';
import path from 'path';
import { getSimilarItems } from '../utils/vectorStore';
import { embedText, generateChatCompletion } from '../utils/openaiClient';

/**
 * Actual system retrieval function that connects to the vector store
 */
async function retrievalFunction(query: string): Promise<any[]> {
  try {
    ...
    let priorityInfoType: string | undefined;
    ...
    const retrievedItems = getSimilarItems(
      queryEmbedding,
      10, // Retrieve 10 items
 ...
 */
async function answerGenerationFunction(
  query: string, 
...
Answer:`;
    ...
    const answer = await generateChatCompletion(systemPrompt, userPrompt, 'gpt-4');
...
async function runBaselineEvaluation() {
  ...
  console.log(`Evaluating ${TEST_QUERIES.length} test queries...`);
  const fullResults = await runFullEvaluation(
    TEST_QUERIES,
    retrievalFunction,
  ...
  const resultsFile = path.join(resultsDir, `baseline_evaluation_${timestamp}.json`);
