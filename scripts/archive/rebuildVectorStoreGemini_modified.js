 ...
 * 
 * This script rebuilds the vector store using Gemini embeddings through the
 * centralized embedding client and stores the results in Supabase.
 * 
...
import { getEmbeddingClient, embedText, embedBatch } from '../utils/embeddingClient.js';
...
async function extractDocumentContext(text, metadata) {
  try {
    ...
    const prompt = `
      ...
      - entities: Array of key entities (people, companies, products) mentioned
      - documentType: Type of document (e.g., technical documentation, tutorial)
      - technicalLevel: Number from 0-3 indicating technical complexity
      - audienceType: Array of target audiences
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          logger.warning(`Extracted JSON still invalid, falling back to regex extraction: ${innerError.message}`);
          // Continue to regex extraction
        }
      }
      
      // Use regex to extract individual fields
      return extractBasicContext(responseText, metadata);
    }
  } catch (error) {
    logger.error('Error extracting document context with Gemini', error);
    ...
    for (let i = 0; i < textChunks.length; i += chunkBatchSize) {
      ...
      for (let j = 0; j < chunkBatch.length; j++) {
        ...
        try {
          ...
          // Generate embedding using the centralized client with proper TaskType
          let embedding;
          try {
            embedding = await embeddingClient.embedText(textToEmbed);
          } catch (embedError) {
            logger.error(`Error generating embedding for chunk ${chunkIndex}`, embedError);
            // Use zero vector as fallback
            const dimensions = embeddingClient.getDimensions();
            embedding = new Array(dimensions).fill(0);
          }
          
          // Store entities in a standard format for easier querying
          const entities = documentContext.entities || [];
          ...
          const chunkToInsert = {
            ...
            metadata: {
              ...
              technical_level: documentContext.technicalLevel,
              topics: Array.isArray(documentContext.mainTopics) ? documentContext.mainTopics : [],
              entities: entities, // Store as a proper array
            },
...
async function processBatch(documents, logger) {
  ...
  const embeddingClient = getEmbeddingClient();
  const dimensions = embeddingClient.getDimensions(); // Get dimensions from client
  ...
  // Generate embeddings in batch with the proper TaskType
  let embeddings;
  try {
    embeddings = await embeddingClient.embedBatch(textsToEmbed, 'RETRIEVAL_DOCUMENT');
    logger.info(`Generated ${embeddings.length} embeddings in batch`);
  } catch (error) {
    logger.error('Error generating batch embeddings', error);
    // Fallback to generate zero vectors
    embeddings = textsToEmbed.map(() => new Array(dimensions).fill(0));
  }
...
async function main() {
  ...
  logger.info('Starting vector store rebuild with Gemini embeddings');
  
  // Initialize the embedding client once
  const embeddingClient = getEmbeddingClient();
  logger.info(`Using embedding client: ${embeddingClient.getProvider()} with ${embeddingClient.getDimensions()} dimensions`);
